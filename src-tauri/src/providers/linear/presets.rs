...
//! 특히 "완료되지 않은"을 표현한 `state.type: { nin: [...] }`가 옳은지는
//! `WorkflowState.type`의 실제 값을 모르므로 확신할 수 없다. 그래서
...
/// "완료됐다"로 볼 상태 타입들.
///
/// # ★ 미검증 — `nin`(제외)인 이유가 여기 있다
///
/// `WorkflowState.type`의 실제 문자열 값을 실측하지 못했다. Linear 문서와 커뮤니티
/// 자료가 이 다섯을 쓰지만 확인은 못 했다.
///
