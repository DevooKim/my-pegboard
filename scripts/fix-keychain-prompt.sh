...
#
# 앱을 재빌드하면 바이너리가 바뀌고 cdhash도 바뀐다. 새 해시는 partition list에
...
# 개인 기기의 개인 Jira 토큰이라 받아들일 만한 거래지만, 공용 기기라면 하지 말 것.
#
# 근본 해결은 Apple Developer 계정으로 Team ID를 얻어 keychain access group을
# 쓰는 것이다(연 $99). 그때는 재서명해도 신원이 유지된다.
set -euo pipefail
