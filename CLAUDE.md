...
| 항목 | 상태 |
|---|---|
| 다중 보드 | **있음** — 타이틀바 탭. 탭이 1개면 탭 바를 숨긴다 (DECISIONS 14 개정) |
...
필요하다 (DECISIONS 14.1). 그렇다고 보드를 늘려 위젯을 더 담으라는 뜻은 아니다:
**보드당 위젯은 적게 유지하는 것이 맞다.**
...
**보드를 만질 때 지킬 것:**
- **비활성 보드는 렌더하지 않는다(언마운트).** 폴링은 데이터 훅의
  `setInterval`이고 언마운트에서 `clearInterval`한다 → 언마운트가 폴링 중단이자
  캐시-즉시-표시다. `display:none`으로 감추면 안 보는 보드가 계속 API를 때린다
- **비활성 보드의 에러 상태를 표시하지 말 것.** 폴링하지 않으므로 알 수 없다.
...
- 단축키: `⌘,` 설정 / `⌘R` 전체 새로고침 / `⌘N` 위젯 추가 / `Esc` 모달 닫기
  / `⌘⇧N` 티켓 생성(첫 Jira 위젯) / **`⌘1`~`⌘9` 보드 전환** (탭에서는 `←` `→`)
